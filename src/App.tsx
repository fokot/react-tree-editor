import React, {createContext, Dispatch, SetStateAction, useCallback, useContext, useReducer, useState} from "react";
import './App.css';
...
const TextElem = ({element, deleteCell, updateCell}: {
  ...
  // const [state, dispatch] = useReducer(element);
  const [editing, setEditing] = useContext(EditingContext)!;
  // const [state, setState] = useState(element.text);
  //FIXME tu pokracovat
  return <div style={{float: "left"}}>📝
    <input id={element.id} value={element.id === editing.id ? editing.text : element.text}
           ...
           // setEditing({id: element.id, text: e.target.value})}}
...
const TreeEditor = ({model, setModel}: {
  model: Table[],
  setModel: Updater<Table[]>,
}) => {
  ...
  return (<div className="App" style={{display: "flex", flexDirection: "column", gap: "20px"}}>
...
type UseEditingReturnType = ReturnType<typeof useEditing>;

export const EditingContext = createContext<UseEditingReturnType | null>(null);
